...
OptionParser.new do |opts|
  opts.banner = "Usage: #{$PROGRAM_NAME} [options] input_folder_or_files... output"
  ...
  opts.on("--size=[WIDTHxHEIGHT]", "Output width (default: #{options.output_width}x#{options.output_height})") do |s|
    ...
    options.output_height = size[1].to
  end
  ...
  opts.on("--zoom-direction=[DIRECTION]", ["random"] + ["top"].product(["left", "center", "right"], ["in", "out"]).map {|m| m.join("-")}, "Zoom direction (default: #{options.zoom_direction})") do |t|
    options.zoom_direction = t
  end
  opts.on("--zoom-rate=[RATE]", Float, "Zoom rate (default: #{options.zoom_rate})") do |n|
    options.zoom_rate = n
  end
  opts.on("--scale-mode=[SCALE_MODE]", [:pad, :pan, :crop_center], "Scale mode (pad, crop_center, pan) (default: #{options.scale_width})") do |n|
...
if ARGV.length < 2
  puts "Usage: #{$PROGRAM_NAME} [options] input_folder_or_files... output"
  exit 1
end
...
# Determine if the input is a directory or a set of files
if File.directory?(ARGV[0])
  input_folder = ARGV[0]
  input_files = Dir.glob(File.join(input_folder, '*.{jpg,jpeg,png,gif}')).sort
elsif ARGV[0].include?('.')
  ARGV.each do |input|
    if File.exist?(input) && input.match?(/\.(jpg|jpeg|png|gif)$/i)
      input_files << input
    else
      puts "Input file #{input} does not exist or is not an image file."
      exit 1
    end
  end
  input_files.sort!
else
  puts "Invalid input provided. Please provide a directory or list of image files."
  exit 1
end

if input_files.empty?
  puts "No imagep files found or provided."
  exit 1
end
